...
\begin{itemize}
\item \TTBF{NON} (négation) : $ \neg $      \hspace*{2cm}  (autre notation : $ \overline{\text{A}} $ )
\item \TTBF{ET} (conjonction) : $ \wedge $  \hspace*{2cm}  (autre notation : $ \text{A} \cdot \text{B} $ )
\item \TTBF{OU} (disjonction) : $ \vee $    \hspace*{2cm}  (autre notation : $ \text{A} + \text{B} $ )
\end{itemize}
...
\begin{itemize}
\item \TTBF{ET} ($ \wedge $) est parfois aussi symbolisé comme une multiplication
\item \TTBF{OU} ($ \vee $) est parfois aussi symbolisé comme une addition
\end{itemize}

En effet, lorsque l'on cherche à tester plusieurs cas, si des \TTBF{OU} logiques s'enchaînent, alors il faut juste s'assurer qu'au moins l'un d'eux soit \textit{vrai}.
...
\`A l'inverse, si l'on observe la même chose sous forme de conjonction de cas (donc avec des \TTBF{ET}), comme au moins un cas est \textit{faux}, le résultat total est également \textit{faux} et peut être vu comme un $ 0 $ :
...
Il est important également de noter que lorsque vous écrirez des conditions complexes (c'est-à-dire constituées de plusieurs comparaisons), vous vous rendrez compte que vous pourrez rédiger ces conditions sous forme de \textit{conjonctions de cas} (c'est-à-dire un enchaînement principal de \TTBF{ET}) ou sous forme de \textit{disjonctions de cas} (c'est-à-dire un enchaînement principal de \TTBF{OU}), et surtout, qu'il est possible de passer de l'un à l'autre par des négations.
...
Distribuer la négation implique que les \TTBF{ET} deviennent des \TTBF{OU} (et inversement), puis que chaque membre subisse une négation.
%Vous vous rendrez compte que vos conditions peuvent s'écrire de la même façon : choisissez celle que vous préférez ou celle qui serait plus optimale.
