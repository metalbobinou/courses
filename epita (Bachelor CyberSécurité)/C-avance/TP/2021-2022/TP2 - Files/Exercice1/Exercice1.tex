...
Vous devez maintenant écrire le (ou les) \textit{makefile(s)} de votre projet (et éventuellement un fichier \texttt{configure}) afin de produire une bibliothèque statique nommée \TTBF{libmyqueue.a} et une bibliothèque dynamique nommée \TTBF{libmyqueue.so}.
...
À la place, la cible \textit{static} de votre makefile devra produire une bibliothèque statique nommée \TTBF{libmyqueue.a}, et la cible \textit{shared} devra produire une bibliothèque dynamique nommée \TTBF{libmyqueue.so}.
