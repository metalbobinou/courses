...
\ExoSpecsCustom{\TTBF{Makefile}}{\TTBF{\RenduDir/}}{750}{640}{Fonctions recommandées}{\TTBF{gcc(1)}, \TTBF{ar(1)}}
...
Vous devez écrire le (ou les) \textit{makefile(s)} de votre projet (et éventuellement un fichier \texttt{configure}) afin de produire une bibliothèque statique nommée \TTBF{libmylinkledlist.a} et une bibliothèque dynamique nommée \TTBF{libmylinkledlist.so}.
...
À la place, la cible \textit{static} de votre makefile devra produire une bibliothèque statique nommée \TTBF{libmylinkledlist.a}, et la cible \textit{shared} devra produire une bibliothèque dynamique nommée \TTBF{libmylinkledlist.so}.
